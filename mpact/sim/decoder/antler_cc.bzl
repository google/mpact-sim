...
def antlr4_cc_parser(
    ...
    native.cc_library(
        ...
        srcs = [f for f in out_files if f.endswith(".cpp")],
        hdrs = [f for f in out_files if f.endswith(".h")],
        deps = ["//third_party/antlr/v4:cc_runtime"] + deps,
        copts = [
            "-fexceptions",
            "-Wno-nonnull",
        ],
...
def _make_tool_invocation_command(package, listener = False, visitor = False):
    ...
    return "java -jar $(location @org_antlr_tool//antlr-runtime-4.10.1.jar) " + \
           "$(SRCS)" + \
...
def _make_google3_cleanup_command(out_files):
    ...
    return ";\n".join([
        ";\n".join([
            "sed -i '0,/antlr4-runtime.h/s//third_party\\/antlr\\/v4\\/cpp\\/antlr4-runtime.h/' $(@D)/%s" % filepath,
